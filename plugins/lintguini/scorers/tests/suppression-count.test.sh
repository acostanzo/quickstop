...
# js-bait: 5 .js files × 12 markers each → 60 (above threshold_high=50)
out=$(triple_run "$FIXTURES/js-bait")
assert_eq "js-bait language"     "javascript" "$(echo "$out" | jq -r .evidence.language)"
assert_eq "js-bait suppressions" "60"         "$(echo "$out" | jq -r .evidence.suppressions)"
assert_eq "js-bait scanned"      "5"          "$(echo "$out" | jq -r .evidence.files_scanned)"
