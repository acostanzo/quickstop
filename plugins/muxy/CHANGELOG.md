...
### Changed
- Bundled tmux MCP server directly in plugin.json
  - MCP server starts automatically when plugin is enabled
  - No manual installation required
- Updated `/muxy:doctor` command to reflect bundled MCP server
  - Simplified troubleshooting (just restart Claude Code if MCP not available)
