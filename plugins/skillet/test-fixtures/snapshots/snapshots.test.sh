#!/usr/bin/env bash
# snapshots.test.sh — invariant B regression suite for skillet's M2 migration.
#
# For each fixture under snapshots/{clean,mid,noisy}/:
#   1. Run score-skillet.sh against the fixture's input directory
...
#   5. Confirm `.observations | length == 5` AND the observation IDs
#      match the M2 contract in the rubric-stanza order.
#
...
SCORER="$REPO_ROOT/plugins/pronto/agents/parsers/scorers/score-skillet.sh"
...
mid_worktree_setup() {
  ...
  MID_WORKTREE="$(mktemp -d -t skillet-snap-mid.XXXXXX)"
...
run_fixture() {
  ...
  # 4. observations[] length == 5.
  local olen
  olen=$(echo "$actual" | jq -r '.observations | length // 0')
  if [[ "$olen" == "5" ]]; then
    note_pass
  else
    note_fail "$slug: observations length expected 5 got $olen"
  fi

  # 5. observations[] ID set matches the M2 contract (exact set, exact order).
  local expected_ids actual_ids
  expected_ids='["skill-frontmatter-completeness-ratio","skill-skeletal-count","skill-todo-marker-count","skill-broken-references-count","skill-stray-file-count"]'
