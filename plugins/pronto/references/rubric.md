...
The bands are calibrated against the nine-fixture set lintguini ships in 2b3 — `<lang>-{low,mid,high}` for python, ruby, typescript. Hand-walked verification table: python-low (0.25, 0, 0.00, 60) → 30/0/30/50 mean 28; python-mid (0.50, 1, 1.00, 2) → 50/100/100/95 mean 86; python-high (1.00, 1, 1.00, 0) → 100/100/100/100 mean 100; ruby-mid (0.60, 1, 1.00, 2) → 70/100/100/95 mean 91; typescript-mid (0.33, 1, 1.00, 2) → 30/100/100/95 mean 81. The remaining six (ruby-low, ruby-high, typescript-low, typescript-high) follow the same shape — low fixtures land at 28 (F band), high fixtures at 100 (A+ band). These per-fixture means stay calibrated to four observations because `lint-pass-rate` empty-scopes on every existing fixture (none carry the lintguini provenance marker the conditional scorer gates on); the four legacy observations remain the only contributors and the table above is unchanged.

`linter-strictness-ratio`, `ci-lint-wired-ratio`, and `lint-pass-rate` share the five-band shape from inkwell's 2a3 stanza for `readme-arrival-coverage`: `gte 1.00 → 100` rewards meeting the language baseline (or the single-CI-surface fully wired, or every scanned file clean), `gte 0.40 → 50` floors a half-baselined linter at presence-only territory, and the `else 30` band catches the genuinely-loose cases (typescript-mid's 1/6 strict-flag ratio of 0.33 lands here). The three ratios share a shape because they share a semantic — all ask "what fraction of the baseline is met?" against a per-language, per-CI-surface, or per-scanned-file yardstick.

`lint-pass-rate` comes from `score-lint-pass-rate.sh` (lintguini 0.5.0+), which actually runs the configured linter on the consumer repo and grades by `files_clean / files_scanned`. The scorer is conditional — it gates on the `Generated by lintguini` provenance marker and empty-scopes on non-lintguini consumers per ADR-008 §5 (audit grades against the rubric, not against templates). Every existing 2b3 fixture lands in the empty-scope branch because none carry the provenance marker; the band calibration is therefore preliminary and may tighten in a future plan once we have lintguini-managed fixture data to calibrate against.
...
`lintguini` emits the four legacy observation IDs natively from 2b2 onward, with `lint-pass-rate` added in lintguini 0.5.0 (T5 of the lintguini-expansion plan); 2b3 wires the rubric stanza so the translator path drives the dimension score and the orchestrator's transitional inline math is retired (see `phase-2-2b3-lintguini-contract-fixtures.md`).
