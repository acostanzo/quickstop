...
Lint-posture toolkit for Claude Code consumer repos: configure, lint, format, fix. Pronto-sibling audit grades lint-posture against the rubric.

- Five skills: `/lintguini:configure` (write rubric-aligned lint/format configs), `/lintguini:lint` (run linters, surface findings), `/lintguini:format` (run formatters, `--check` for diff-only), `/lintguini:fix` (auto-fix wrapper with `--apply` / `--apply-semantic` split), `/lintguini:audit` (Pronto-sibling lint-posture auditor)
- Six first-class language paths: python (ruff), javascript / typescript (biome or eslint+prettier), rust (rustfmt + clippy), ruby (rubocop or standardrb), go (golangci-lint + gofmt)
- Per-language config templates derived from `plugins/pronto/references/roll-your-own/lint-posture.md` — the rubric is the source of truth (ADR-008)
- Self-describing configs — every generated config carries a `Generated by lintguini` provenance comment pointing at the rubric
- Wire-contract v2 envelope on `/lintguini:audit --json` — five observations (four legacy + lint-pass-rate conditional on lintguini-managed config)
- Declares `compatible_pronto: ">=0.2.0"` per ADR-004 handshake

**Commands:** `/lintguini:configure`, `/lintguini:lint`, `/lintguini:format`, `/lintguini:fix`, `/lintguini:audit`
