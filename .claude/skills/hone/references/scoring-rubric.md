...
| Hook Quality | 10 | Event types, timeouts, matchers, no duplication, hook invariants |
| Documentation | 10 | README quality, inline docs, usage examples, surface declaration |
| Over-Engineering | 10 | Agent sprawl, verbose instructions, unnecessary complexity |
| Security | 10 | Secrets scan, tool restrictions, permission scope, consumer-artefact mutation |
...
**What:** Hook definitions — event types, timeouts, matchers, no duplication of built-in behavior, hook invariants.
...
#### Hook Invariant Deductions

| Issue | Points | Severity | Description |
|-------|--------|----------|-------------|
| Hook returns `updatedInput`, `updatedOutput`, `decision`, `behavior`, or `permissionDecision` | -25 each (max -50) | Critical | Invariant 1 — payload/flow mutation |
| Hook installs persistent host state (`npm install`, `brew install`, `pip install`, `cargo install`, `go install`, `systemctl enable`, `launchctl load`, `sudo`) | -15 each (max -30) | High | Invariant 2 — persistent host state |
| Hook writes to Tier 1 literal paths (`/etc/`, `/usr/local/`, `~/.bashrc`, `~/.zshrc`, `~/.gitconfig`, `~/.profile`) | -15 each (max -30) | High | Invariant 3 — undeclared write |
| `hooks/hooks.json` declares an event with no corresponding script | -5 each | Low | Manifest drift |

Hook invariant deductions stack with existing Hook Quality deductions. Category score is floored at 0 after stacking.
...
**No hooks = neutral (100):** If a plugin has no hooks and doesn't need them, score stays at 100. Hook invariant deductions and the manifest-drift deduction are skipped when no `hooks/` directory is present.

## Category: Documentation (10%)

**What:** README quality, usage examples, installation instructions, surface declaration.
...
#### Surface Declaration Deductions

| Issue | Points | Severity | Description |
|-------|--------|----------|-------------|
| README missing "Plugin surface" section | -5 | Low | Capability enumeration |
| Hooks declared in `hooks/hooks.json` but absent from README's surface enumeration | -5 each (max -15) | Low | Surface visibility |
...
**What:** Secrets in files, tool restrictions, permission scope, silent consumer-artefact mutation.
...
#### Consumer-Artefact Mutation Deductions
...
| Issue | Points | Severity | Description |
|-------|--------|----------|-------------|
| Plugin code (Scope A) calls `gh pr edit --body-file/-F/-B` | -25 each | Critical | Silent PR-body mutation |
| Plugin code (Scope A) calls `git config --global` | -25 each | Critical | Consumer config mutation |
| Plugin code (Scope A) calls `gh repo edit` | -25 each | Critical | Consumer repo mutation |
| Plugin code (Scope A) calls `gh release create` or `gh release edit` | -25 each | Critical | Consumer release mutation |
